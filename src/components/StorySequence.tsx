          ...
          <ol className="audience-scenes">
            {slides.map((slide, index) => (
              <li className="audience-scene" id={slide.id} key={slide.id} data-current={index === active} aria-hidden={pinned && index !== active ? true : undefined} inert={pinned && index !== active}>
                ...
                <div className="audience-art">{slide.media ?? (slide.image ? <Image src={slide.image} width={960} height={1200} alt="" sizes="(max-width: 767px) 260px, (max-width: 1023px) 45vw, 48vw" loading={slide.loading ?? "lazy"} /> : null)}</div>
              </li>
            ))}
          </ol>
