...
export function AudienceStory({ locale, children }: { locale: Locale; children: ReactNode }) {
  const slides: StorySlide[] = audience.map((item, index) => ({
    ...
    image: `/art/cutouts/audience/${String(index + 1).padStart(2, "0")}.webp`, content: <p key={`fit-copy-${index}`}>{pick(item, locale)}</p>,
  }));
  slides.push({
    id: "fit-case-10",
    label: locale === "ar" ? "عندك سيستم لكن استخدامه معقد" : "Your current system is difficult to use",
    image: "/art/cutouts/audience/10.webp",
    loading: "eager",
    content: (
      <p key="fit-copy-10">
        {locale === "ar"
  ...
  slides.push({ id: "fit-case-11", label: locale === "ar" ? "سهولة استخدام النظام" : "An approachable system", image: "/art/cutouts/audience/11.webp", content: <div key="fit-copy-11">{children}</div> });
...
export function SetupStory({ locale }: { locale: Locale }) {
  const slides = setup.map((item, index) => ({
    id: `setup-step-${index + 1}`, label: pick(item.title, locale), image: `/art/cutouts/setup/${String(index + 1).padStart(2, "0")}.webp`,
...
export function StrategyStory({ locale, chapters }: { locale: Locale; chapters: Chapter[] }) {
  const slides = chapters.map((item, index) => ({
    id: item.id, label: pick(item.title, locale), image: `/art/cutouts/strategies/${String(index + 1).padStart(2, "0")}.webp`,
